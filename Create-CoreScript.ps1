# First, copy the entire script content from the artifact above
# Then paste it into this variable (between the @' and '@ markers)
$scriptContent = @'
# Script to create the core script with proper encoding
$coreScriptPath = "C:\Scripts\00-AD-Assessment-Core.ps1"

# Remove existing file if present
if (Test-Path $coreScriptPath) {
    Remove-Item $coreScriptPath -Force
}

# Create the core script content
$content = @'
...
# Save the content with ASCII encoding to avoid smart quotes
$content | Out-File -FilePath $coreScriptPath -Encoding ASCII -Force

Write-Host "Core script created successfully at: $coreScriptPath" -ForegroundColor Green
Write-Host "File size: $((Get-Item $coreScriptPath).Length) bytes" -ForegroundColor Yellow

# Test that the file is valid PowerShell
try {
    $null = Test-Path $coreScriptPath -ErrorAction Stop
    $testContent = Get-Content $coreScriptPath -Raw
    [scriptblock]::Create($testContent) | Out-Null
...
$scriptContent | Out-File -FilePath "C:\Scripts\00-AD-Assessment-Core.ps1" -Encoding ASCII -Force
